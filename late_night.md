...
- При деплое GitHub OAuth callback нужно менять с `localhost` на другой домен backend-а.
- Скрытые контакты в поиске и профиле больше нельзя выделить, перетащить или скопировать через обычный UI.
- Модалки редактирования и вступления в команду вынесены в portal на `document.body`, чтобы fixed-слой не ломался из-за page-transition transform и не уезжал ниже viewport.
...
- [x] Локально проверить `GET /health`, `POST /auth/logout`, `GET /auth/me` и cookie-очистку на JWT-сессии.
- [x] Локально проверить smoke на временном Postgres с seed-данными.
- [x] Локально проверить live YandexGPT scoring через `SMOKE_YANDEXGPT=1`.
- [x] Локально проверить, что публичные экраны и мобильная вёрстка проходят headless smoke.
- [x] Поднять локальный `docker compose`-стек и проверить backend против postgres-контейнера вместо fallback.
- [x] `PUT /api/v1/profile` теперь принимает пустой `bio` и пустые поля контактов без 500.
- [x] В compose-контуре проверены `GET /api/v1/auth/me`, `GET /api/v1/profile`, `GET /api/v1/users`, `GET /api/v1/teams` и `GET /api/v1/applications` с JWT-cookie.
...
- [x] Текущий локальный `.env` не поднимает Postgres, поэтому standalone `/health` на нём деградирует до `503`, а релизный прогон нужно делать на temp Postgres или живой БД.
- [x] Для compose-проверки на этой машине сначала нужно запустить Docker Desktop: CLI установлен, но Docker engine сейчас недоступен.
- [x] В compose-контуре backend теперь стартует с контейнерным Postgres и отвечает `GET /health` как `ok`.
...
- [x] Формы создания и редактирования команды показывают обязательные и необязательные поля, а валидация теперь возвращает понятные ошибки по конкретным полям.
- [x] В форме команды поле `Хакатон` переименовано в `Событие`, звёздочки в бейджах выровнены, а роли принимаются без привязки к регистру.
