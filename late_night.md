...
- [ ] Сделать понятный empty/onboarding state для первого OAuth-пользователя.
...
- [ ] Проверить Postgres, OAuth, YandexGPT и healthcheck на живом запуске.
- [x] Зафиксировать smoke-процесс как обязательную проверку перед показом.
- [ ] Чтоб при входе в окно создания хакатона нельзя было прокручивать остальной фон позади окна
- [ ] Лимит скоринга исчерпан. Следующая попытка: 2026-03-28T19:49:59.557Z. Вот тут сократить дату до минуты, меньшее не показывать
...
- [ ] Сделать GitHub import понятным и заметным в интерфейсе.
- [ ] Решить, какие API оставить как внутренние, а какие показать пользователю.
- [ ] Проверить все списки и карточки на реальных данных, а не на пустых состояниях.
- [ ] Упростить paywall до аккуратного placeholder-сценария.
...
- [ ] Пометить старые pitch/hackathon материалы как legacy.
- [ ] Убрать лишнее дублирование между документами.
- [ ] Оставить один источник правды для релиза и один для локального запуска.
- [ ] Поменять в хедере хакатон-матчинг на айти-матчинг и уменьшить расстояние между символами
- [ ] Добавить поддержку правильного количества участников, не 1 участников а 1 участник и т.д
