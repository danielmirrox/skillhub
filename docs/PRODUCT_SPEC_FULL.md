...
**Версия:** 1.1  
**Дата:** 22 марта 2026  
**Статус:** Спецификация для разработки MVP и пост-MVP, синхронизированная с текущей реализацией
...
| Фильтры поиска | ✓ | ✓ |
| Избранное и голоса за участников | ✓ | ✓ |
...
| avatar_url | VARCHAR(500) | URL аватара из GitHub OAuth; если пусто, UI показывает default avatar |
...
| status | ENUM('active','paused','closed') | Состояние команды |
| updated_at | TIMESTAMP | |
...
**Текущее поведение в коде:** закрытые команды скрываются из общего списка, но остаются видны капитану в собственном списке; при принятии заявки свободный слот уменьшается, а при удалении участника слот возвращается. Если команда закрыта или заполнена, новая заявка или её принятие получают 409.
...
| GET | `/users/favorites` | Список избранных участников | ✓ |
| POST | `/users/:id/favorite` | Добавить или убрать из избранного | ✓ |
| POST | `/users/:id/vote` | Голос за участника (+1 / -1) | ✓ |
...
- Избранное и голосование используются в UI-карточках и могут менять счётчики без перезагрузки всей страницы.
- При возврате ответа с сервера UI пересортировывает список, чтобы новые значения сразу отражались в выдаче.
...
| DELETE | `/teams/:teamId/members/:userId` | Удалить участника из команды (только капитан) | ✓ |
...
`status` поддерживает `active`, `paused`, `closed`. Закрытая команда скрывается из общей ленты и не принимает новые заявки.
`updatedAt` выставляется сервером автоматически.

**Текущее поведение в коде:** при принятии заявки `slotsOpen` уменьшается, при удалении участника увеличивается. Если команда закрыта или у неё закончились слоты, новые заявки получают 409.
...
**Ограничения:** команда должна быть активна и иметь свободные слоты. Если команда закрыта или заполнена, сервер возвращает 409.
...
```json
{
  "incoming": [
    {
      "id": "uuid",
      "team": { "name": "...", "hackathonName": "...", "status": "active", "isActive": true, "slotsOpen": 1 },
...
3. Нормализует репозитории: принимает только реальные `github.com/<owner>/<repo>` URL, игнорирует profile README и мусорные ссылки.
4. Возвращает `suggestedPrimaryStack` и `suggestedProjectLinks`, но **не перезаписывает профиль автоматически**.
5. Пользователь подтверждает изменения через `PUT /profile`.
